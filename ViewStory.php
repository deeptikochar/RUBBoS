...
function display_follow_up($cid, $level, $display, $filter, $link, $comment_table)
{
 // $follow = mysql_query("SELECT story_id,id,subject,writer,date FROM $comment_table WHERE parent=$cid", $link) or die("ERROR: Query failed");
  $follow = $link->query("SELECT story_id,id,subject,writer,date FROM $comment_table WHERE parent=$cid;")or die("ERROR: Query failed");
...
//    $result = mysql_query("SELECT * FROM stories WHERE id=$storyId") or die("ERROR: Query failed");
    $result = $link->query("SELECT * FROM stories WHERE id=$storyId;") or die("ERROR: Query failed");
    if (count($result) == 0)
    {
     // $result = mysql_query("SELECT * FROM old_stories WHERE id=$storyId") or die("ERROR: Query failed");
    $result = $link->query("SELECT * FROM old_stories WHERE id=$storyId;") or die("ERROR: Query failed");  
  ...
  //  while ($count_row = mysql_fetch_array($count_result))
   ...
   $comment = $link->query("SELECT * FROM $comment_table WHERE story_id=$storyId AND rating>=$filter allow filtering;") or die("ERROR: Query failed");
